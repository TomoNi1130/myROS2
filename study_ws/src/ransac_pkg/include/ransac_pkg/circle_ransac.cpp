...
RANSAC::RANSAC(int max_iterations, double threshold, std::vector<double> _x, std::vector<double> _y)
    : max_iterations(max_iterations), threshold(threshold), _x(_x), _y(_y)
{
      inlier.resize(_x.size(), false);
}

std::vector<circle> RANSAC::circle_ransac(const int circle_num)
{
      ...
      for (int now_circle_num = 0; now_circle_num < circle_num; now_circle_num++)
      {
            ...
            for (int i = 0; i < max_iterations; i++)
            {
                  ...
                  {
                        double Score = guess_inlier_count / (guess_circle.radius + 0.8);
                        // double mattch = guess_circle.radius / 3.0;
                        // double Score = abs(1 - mattch);
                        if (best_Score < Score)
                        {
